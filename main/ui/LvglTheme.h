...
public:
  ...
  LvglTheme(std::string name, const Palette &palette, const Metrics &metrics, const Fonts &fonts)
      : name_(std::move(name)), palette_(palette), metrics_(metrics), fonts_(fonts) {
    buildDefaultStyles();
  }

  // optional convenience constructor with defaults
  explicit LvglTheme(std::string name) : name_(std::move(name)) { buildDefaultStyles(); }
...
private:
  ...
  void buildDefaultStyles() {
    ...
    defineStyle("button.pressed").bgColor(lv_palette_darken(LV_PALETTE_BLUE, 2)).textColor(lv_color_white());

    defineStyle("button.disabled").bgColor(palette_.surface).textColor(lv_palette_lighten(LV_PALETTE_GREY, 2));
    defineStyle("button.secondary").bgColor(palette_.secondary).textColor(lv_color_white());

    defineStyle("button.primary").bgColor(palette_.primary).textColor(lv_color_white());

    // ---- LABEL ----
    defineStyle("label.main").textColor(palette_.text).textFont(fonts_.medium);

    defineStyle("label.title").textColor(palette_.primary).textFont(fonts_.xlarge);

    defineStyle("label.muted").textColor(palette_.text).textFont(fonts_.small);

    // ---- CHECKBOX ----
    defineStyle("checkbox.main").textColor(palette_.text).padAll(metrics_.padding / 2);

    defineStyle("checkbox.checked").bgColor(palette_.primary);

    defineStyle("checkbox.disabled").textColor(lv_palette_lighten(LV_PALETTE_GREY, 2));
    ...
    defineStyle("dropdown.selected").bgColor(palette_.primary).textColor(lv_color_white());

    defineStyle("screen.main").bgColor(palette_.background).textColor(palette_.text);
