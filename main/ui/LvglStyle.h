...
class LvglStyle {
public:
  LvglStyle() { lv_style_init(&style_); }
  ...
  LvglStyle &borderWidth(int32_t width) {
    border_width_ = width;
  ...
  LvglStyle &shadowWidth(int32_t width) {
    shadow_width_ = width;
  ...
  //
  void applyTo(lv_obj_t *obj, lv_style_selector_t selector = 0) const {
    if (!obj)
      return;
    lv_obj_add_style(obj, const_cast<lv_style_t *>(&style_), selector);
  }
...
private:
  void copyCacheFrom(const LvglStyle &other) {
    ...
    *this = LvglStyle(); // re-init
    if (other.has_bg_color_)
      bgColor(other.bg_color_);
    if (other.has_bg_opa_)
      bgOpacity(other.bg_opa_);
    if (other.has_text_color_)
      textColor(other.text_color_);
    if (other.has_font_)
      textFont(other.font_);
    if (other.has_border_color_)
      borderColor(other.border_color_);
    if (other.has_border_width_)
      borderWidth(other.border_width_);
    if (other.has_radius_)
      radius(other.radius_);
    if (other.has_pad_all_)
      padAll(other.pad_all_);
    if (other.has_shadow_color_)
      shadowColor(other.shadow_color_);
    if (other.has_shadow_width_)
      shadowWidth(other.shadow_width_);
  }
