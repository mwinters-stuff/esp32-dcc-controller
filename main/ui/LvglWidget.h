#ifndef _LVGL_WIDGET_H
#define _LVGL_WIDGET_H
...
public:
    ...
    void setFont(const lv_font_t* font) {
        if (obj_) lv_obj_set_style_text_font(obj_, font, LV_PART_MAIN);
    }

    void setColor(lv_color_t color) {
        if (obj_) lv_obj_set_style_text_color(obj_, color, LV_PART_MAIN);
    }

    void setVisible(bool visible) {
        if (obj_) lv_obj_add_flag(obj_, visible ? LV_OBJ_FLAG_HIDDEN : 0);
    }
