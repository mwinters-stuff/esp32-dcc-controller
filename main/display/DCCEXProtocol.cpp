...
static const char *TAG = "DCCEXProtocol";
// DCCEXProtocol class
// Public methods
              ...
              if (!routeListRequired || _receivedRouteList) {
                if (turntableListRequired && !_turntableListRequested) {
                  ...
                  if (!turntableListRequired || _receivedTurntableList) {
                    _receivedLists = true;
                    ESP_LOGI(TAG, "Lists Fully Received");
                  }
                }
              }
            }
...
void DCCEXProtocol::_processRosterEntry() { //<jR id ""|"desc" ""|"funct1/funct2/funct3/...">
  ESP_LOGI(TAG, "processRosterEntry()");
  ...
  if (!missingRosters) {
    _receivedRoster = true;
    ESP_LOGI(TAG, "processRosterEntry(): received all: %d", getRosterCount());
    _delegate->receivedRosterList();
  }

  free(name);
  free(funcs);
  ESP_LOGI(TAG, "processRosterEntry(): end");
}
